...
gf_aggregate <- function(x, ...){
  x %>%
    dplyr::group_by(...) %>%
    summarise(cases = sum(.data$cases),
              deaths = sum(.data$deaths),
              prev = weighted.mean(.data$prev, .data$par),
              life_years = sum(life_years),
              dalys = sum(dalys),
              treatment_coverage = weighted.mean(.data$treatment_coverage, .data$par),
              net_coverage = weighted.mean(.data$net_coverage, .data$par),
              irs_coverage = weighted.mean(.data$irs_coverage, .data$par),
              smc_coverage = weighted.mean(.data$smc_coverage, .data$par),
              rtss_coverage = weighted.mean(.data$rtss_coverage, .data$par),
              iptp_coverage = weighted.mean(.data$iptp_coverage, .data$par),
              iccm_coverage = weighted.mean(.data$iccm_coverage, .data$par),
              diagnostic_and_treatment_cost = sum(diagnostic_and_treatment_cost),
              net_cost = sum(.data$net_cost),
              irs_cost = sum(.data$irs_cost),
              smc_cost = sum(.data$smc_cost),
              rtss_cost = sum(.data$rtss_cost),
              ipti_cost = sum(vipti_cost),
