...
commodities_and_services <- function(x){
  x %>%
    dplyr::mutate(llins_nets_distributed = round(annual_net_distibuted_gts(.data$eq_npc) * .data$par),
           irs_people_protected = round(.data$irs_coverage * .data$par),
           ...
           non_malarial_fever_rdts = round(ifelse(.data$prev >0.05, 0.37 * .data$non_malarial_fevers * .data$proportion_act, 0)),
