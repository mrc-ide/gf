...
gf_global_plan <- gf_optimised %>%
  filter(pre == "gp", replenishment == "gp", post == "gp") %>%
  group_by(year) %>%
...
gts_gf <- bind_row(gf_global_plan, gts_global_plan)
...
cases_increase_budget_prop$cases_upper = cases_increase_budget_prop$cases_upper - cases_increase_budget_prop$cases[cases_upper$budget_prop == 1]
